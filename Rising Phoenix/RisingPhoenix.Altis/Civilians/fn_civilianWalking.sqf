...
private _nearBuildings = _pos nearObjects ["House",300];
if (_nearBuildings isNotEqualTo []) then {
	...
	for "_i" from 0 to ((count _nearBuildings) min 4)-1 do {
		_walkTargets pushBack (getPosATL (_nearBuildings select _i));
	};
};

for "_i" from 0 to 1 do {
	_walkTargets pushBack ([_pos,200] call CBA_fnc_randPos);
};
