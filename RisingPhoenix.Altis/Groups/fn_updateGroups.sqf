...
if !(isNull _display) then {
	...
	if (_oldSelectionPath isNotEqualTo []) then {
		...
		private _roleName = getText(missionConfigFile >> DT_bluforFaction >> "Dynamic_Roles" >> (_oldData select 1) >> "name");
	...
	if (_newSelectionPath isNotEqualTo []) then {
		...
		private _roleName = getText(missionConfigFile >> DT_bluforFaction >> "Dynamic_Roles" >> (_newData select 1) >> "name");
